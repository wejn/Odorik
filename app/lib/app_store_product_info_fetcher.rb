...
@x = AppStoreProductInfoFetcher.new(inapps) do |pi|
	p pi
end

## Asynchronous, wrapped in method call:
AppStoreProductInfoFetcher.fetch(inapps) do |pi|
	p pi
end

## Synchronous:
pi = AppStoreProductInfoFetcher.fetch(inapps)
p pi
...
# }
=end
class AppStoreProductInfoFetcher
	@@mutex = Mutex.new
	@@cache = {}
